...
from lab4.features import FeatureImage
...
def proximity(vector1, vector2):
    return 1 / (1 + distance(vector1, vector2))


feature_names = ['relative_' + name for name in ['weight', 'center_x', 'center_y', 'inertia_x', 'inertia_y']]
target = 'Ξ´ΕΡΩΟΤΙΔΕΝΞ´ΕΡΩΤΙΠΟΤA'


if __name__ == '__main__':
    features = pd.read_csv('../lab4/features.csv')
    sentence = ""
    results = []
    for i in range(len(target) - 1):
        symbol = FeatureImage(Image.open(f'../lab5/img/out/letter_{i}.bmp'), invert=False)
        feature_vector = [
            symbol.relative_weight(),
        ...
        distances = features.apply(
            lambda feature_symbol: distance(feature_symbol[feature_names], feature_vector), axis=1
        )
        proximities = distances.apply(
            lambda distance: 1 - distance / distances.max()
        )
        feature_images = {letter: FeatureImage(Image.open("../lab4/" + filename(i))) for i, letter in enumerate(symbol_set)}
        ...
        potential_letters = proximities[proximities.proximity > delta]
        # print(proximities[proximities.proximity > delta])
        ...
        results.append(proximities.apply(
            lambda guessed_letter:
                ...
                ((guessed_letter['profile_proximity'],) if not pd.isna(guessed_letter['profile_proximity']) else ())
            , axis=1
        ))
        sentence += proximities['letter'][0]
    res = pd.DataFrame(results, index=list(target)[:-2] + ['ΤA'])
    res.to_csv('result.csv')
    print(sentence)
    correct_guesses = 0
    for expected, actual in zip(target, sentence):
        correct_guesses += (expected == actual)
    print('Accuracy:', correct_guesses / len(target))
